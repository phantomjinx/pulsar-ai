...
class PulsarAI {
  constructor() {
    ...
    // Ajouter un listener pour détecter la fermeture de l'onglet
    this.subscriptions.add(
      atom.workspace.onDidDestroyPaneItem(({item}) => {
        if (item instanceof PulsarAIView) {
          this.panel = null;
          // Mettre à jour la config quand le panel est fermé
    ...
    // Ajouter l'observer sur la config
    this.subscriptions.add(
  ...
  async shareSelection() {
    ...
    // Sauvegarder les informations de sélection
  ...
  async explainCode() {
    ...
    try {
      ...
      atom.notifications.addError('Erreur lors de l\'analyse du code', {
  ...
  async optimizeCode() {
    ...
    try {
      ...
      atom.notifications.addError('Erreur lors de l\'optimisation du code', {
        detail: error.message,
        dismissable: true
      });
    }
  ...
  async toggle() {
    ...
    try {
      ...
      // Vérifie si le panneau est actuellement actif
      ...
      if (isPulsarAIActive) {
        // Si on est sur Pulsar AI, on toggle la visibilité du panneau
        const dock = atom.workspace.getRightDock();
        if (dock.isVisible()) {
          dock.hide();
        } else {
        ...
        // Si on n'est pas sur Pulsar AI, on active le panneau et l'onglet
        const dock = atom.workspace.getRightDock();
        if (!dock.isVisible()) {
          dock.show();
        }
