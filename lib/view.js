...
class PulsarAIView {
  ...
  createBufferRange(serializedRange) {
    ...
    try {
      ...
      console.log('Error creating buffer range:', error);
      return null;
    }
  }
  ...
  async addMessage(role, content, metadata = null) {
    console.log('DEBUG - addMessage:', { role, metadata });
    ...
    if (metadata?.codeSelection) {
      ...
      console.log('DEBUG - Updated currentCodeSelection:', this.currentCodeSelection);
    }
  ...
  async renderMessage(message) {
    console.log('DEBUG - renderMessage:', {
      messageRole: message.role,
      messageMetadata: message.metadata,
      currentSelection: this.currentCodeSelection
    });
    ...
    codeBlocks.forEach((block, placeholder) => {
      ...
      console.log('DEBUG - code block:', {
        ...
        hasCodeSelection: !!this.currentCodeSelection,
        currentCodeSelection: this.currentCodeSelection
      });
    ...
    contentElement.querySelectorAll('.btn-apply').forEach(button => {
      ...
      let highlightMarker = null;  // Pour stocker le marqueur

      // Gestionnaire de survol - entrée
      button.addEventListener('mouseenter', () => {
        ...
        if (editor) {
          ...
          // Décorer le marqueur avec une classe CSS
      ...
      button.addEventListener('click', async (e) => {
        try {
          ...
          console.log('DEBUG - Current code selection:', this.currentCodeSelection);
          ...
          // Vérifier si le range est valide
          // Vérifier si le range est valide
          const buffer = editor.getBuffer();
          console.log('DEBUG - Creating range with:', {
            startRow: selection.start.row,
            startCol: selection.start.column,
            endRow: selection.end.row,
            endCol: selection.end.column
          ...
          // Créer des Points d'abord
          const startPoint = new Point(selection.start.row, selection.start.column);
          const endPoint = new Point(selection.end.row, selection.end.column);

          // Puis créer le Range avec ces Points
          const range = new Range(startPoint, endPoint);
          ...
          // Positionner le curseur avec le Point
          editor.setCursorBufferPosition(startPoint);

          // Appliquer les changements
          editor.transact(() => {
            // Récupérer l'indentation de la ligne originale
            const originalIndent = editor.indentationForBufferRow(selection.start.row);

            // Indenter chaque ligne du code à insérer
            const indentedCode = code.split('\n').map(line => {
              if (line.trim() === '') return '';  // Ne pas indenter les lignes vides
              return ' '.repeat(originalIndent * editor.getTabLength()) + line;
            }).join('\n');

            console.log('DEBUG - Indented code:', indentedCode);
