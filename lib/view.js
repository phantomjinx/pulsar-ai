...
class PulsarAIView {
  ...
  setupInputArea() {
    ...
    // Créer le conteneur principal de saisie
    const inputWrapper = document.createElement('div');
    inputWrapper.classList.add('input-wrapper');

    // Nouveau bouton "+ Add context" au début de input-wrapper
    this.contextButton = document.createElement('button');
    this.contextButton.classList.add('pulsar-ai-add-context-button');
    this.contextButton.innerHTML = '<span class="icon icon-plus"></span> Add context';

    // Créer l'instance du navigateur de projet
    this.projectBrowser = new ProjectBrowser();
    document.body.appendChild(this.projectBrowser.element);

    // Ajouter l'événement click
    this.contextButton.addEventListener('click', (e) => {
      e.stopPropagation();
      this.projectBrowser.toggle();
    });
    ...
    this.modelSelect.classList.add('model-select');
    ...
    // Ajouter les éléments à la barre d'outils
    toolbarContainer.appendChild(this.modelSelect);
    toolbarContainer.appendChild(this.sendButton);

    // Assembler les éléments dans l'ordre souhaité
    inputWrapper.appendChild(this.contextButton);
