...
import { Message, PulsarAIState, PulsarAIViewOptions } from './globals'
...
export class PulsarAIView implements ViewModel {
  ...
  private currentCodeSelection: null
  ...
  constructor(options: PulsarAIViewOptions) {
    ...
    const modelProvider = options.state?.modelProvider
    const modelName = options.state?.modelName
    this.setupUI(modelProvider, modelName)

    // // Restore messages if present in state
    if (options.state?.messages) {
      this.messages = options.state?.messages
    //   this.renderMessages()
    }
  }
  ...
  setupUI(modelProvider?: string, modelName?: string) {
    // Create the model Selector
    this.modelSelector = new ModelSelector(modelProvider, modelName)
  ...
  // }
  //
  serialize(): PulsarAIState {
    const state: PulsarAIState =  {
      deserializer: PulsarAIView.SERIALIZATION_ID,

      modelProvider: this.modelSelector.getModelProvider(),
      modelName: this.modelSelector.getModelName(),

      // Messages are already properly processed by addMessage
